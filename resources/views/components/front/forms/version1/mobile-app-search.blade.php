<div class="mixin-container d-xl-none d-flex">
    <div class="drop-container">
        <div class="wrapper-dropdown" id="dropdown4">
            <span class="selected-display black fw-500" id="destination4">{{ !$getCategory ? $defaultContent['lang019'] : $getCategory->getName() }}</span>
            ...
            @foreach($getCategories->where('id','!=',$getCategory?->id) as $mobileCategory)
                <li class="item dark-black">{{ $mobileCategory->getName() }}</li>
            @endforeach
                ...
                @foreach($searchResults as $result)
                    <div class="result-item" style="min-width:100%; padding:10px; border-bottom: 1px solid #eee;">
                        <div class="d-flex align-items-center" style="overflow:hidden; width:100%;">
                            <img src="{{ asset('requisites/logo.png') }}" alt="Logo" style="width: 30px; height: 30px; margin-right: 10px; object-fit: cover;">
                            <span style="font-size: 14px; color: #333; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $result->getName() }}</span>
                        </div>
                        <span class="mt-2 mb-2">ladkfjslkaldsjlkadfsjjfdsa fdfasfasdfsad</span>
                        <span>Price: 20 $</span>
                    </div>
                @endforeach
            </div>
    ...
    <div class="input-field">
        <x-front.forms.form-components.input type="test" is_model="true" is_live="true" model="searchProducts" debounce_time="300ms" id="searchInput2" />
